...
def generate_recommendations(
    ...
    for pattern in patterns[:max_patterns]:
        print(f"  Analyzing pattern: {pattern.pattern_type} | {pattern.account} | {pattern.ticket_count} tickets...")
        try:
            ...
            rec = Recommendation(
                ...
                source_ticket_numbers=[t["ticket_number"] for t in pattern.tickets],
            )
            recommendations.append(rec)

        except Exception as e:
            print(f"  WARNING: Could not generate recommendation for pattern ({pattern.account} / {pattern.issue_type}): {e}")

    recommendations.sort(key=lambda r: ({"high": 0, "medium": 1, "low": 2}.get(r.priority, 1), -r.estimated_monthly_hours_saved))
